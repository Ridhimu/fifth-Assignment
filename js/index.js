...
document.getElementById("donate-now-btn-nuakhali").addEventListener("click",function(){
    ...
    div.innerHTML = `
            <p class ='text-xl font-bold py-4 px-4 border'> You donate ${addMoney} tk for Noakhali Transaction date is ${new Date().toLocaleDateString()}</p>
          `;
...
document.getElementById("donate-btn-for-feni").addEventListener("click",function(){
    ...
    div.innerHTML = `
                <p class ='text-xl font-bold py-4 px-4 border'> You donate ${inputMoney} tk for Feni, Transaction date is ${new Date().toLocaleDateString()}</p>
            `;
    document.getElementById("transaction-container").appendChild(div);
  });
...
document.getElementById("donate-for-student").addEventListener("click", function(){
    ...
    div.innerHTML = `
                <p class ='text-xl font-bold py-4 px-4 border'> You donate ${donateForStudent} tk for Quota Movement, Transaction date is ${new Date().toLocaleDateString()}</p>
            `;
    document.getElementById("transaction-container").appendChild(div);
  });
